...
Expecting one of '${allowedValues.join("', '")}'`);
    ...
    addOption(option) {
      ...
      const handleOptionValue = (val2, invalidValueMessage, valueSource) => {
        ...
        if (val2 !== null && option.parseArg) {
          try {
            val2 = option.parseArg(val2, oldValue);
          } catch (err) {
 ...
 - ${executableDirMessage}`;
    ...
    aliases(aliases3) {
      if (aliases3 === undefined)
        return this._aliases;
      aliases3.forEach((alias) => this.alias(alias));
      return this;
    }
    usage(str) {
      if (str === undefined) {
...
var filterByPosition = function(filter2, elems, data2, options2) {
...
function update(newChilds, parent2) {
  ...
  for (let i = 0;i < arr.length; i++) {
    ...
    if (parent2) {
      node3.prev = arr[i - 1] || null;
      node3.next = arr[i + 1] || null;
    } else {
...
function getLoad(parse6, render3) {
  return function load(content, options3, isDocument2 = true) {
    ...
    function initialize(selector, context, root2 = initialRoot, opts) {
      ...
      let search = selector;
      const searchContext = !context ? rootInstance : typeof context === "string" ? isHtml(context) ? new LoadedCheerio([parse6(context, options4, false, null)], rootInstance, options4) : (search = `${context} ${search}`, rootInstance) : isCheerio(context) ? context : new LoadedCheerio(Array.isArray(context) ? context : [context], rootInstance, options4);
      if (!searchContext)
        return instance;
...
var defaultTreeAdapter = {
  ...
  setDocumentType(document, name, publicId, systemId) {
    const doctypeNode = document.childNodes.find((node3) => node3.nodeName === "#documentType");
    if (doctypeNode) {
      ...
      defaultTreeAdapter.appendChild(document, node3);
    }
  },
...
class Parser {
  constructor(options3, document, fragmentContext = null, scriptHandler = null) {
...
var isVoidElement = function(node3, options3) {
...
var serializeNode = function(node3, options3) {
...
var serializeElement = function(node3, options3) {
...
var serializeAttributes = function(node3, { treeAdapter }) {
  let html10 = "";
...
var serializeTextNode = function(node3, options3) {
...
var serializeCommentNode = function(node3, { treeAdapter }) {
  return `<!--${treeAdapter.getCommentNodeContent(node3)}-->`;
};
var serializeDocumentTypeNode = function(node3, { treeAdapter }) {
...
var adapter = {
  ...
  setDocumentType(document, name, publicId, systemId) {
    ...
    let doctypeNode = document.children.find((node3) => isDirective(node3) && node3.name === "!doctype");
    if (doctypeNode) {
...
var parse8 = getParse((content, options3, isDocument2, context) => options3.xmlMode || options3._useHtmlParser2 ? parseDocument(content, options3) : parseWithParse5(content, options3, isDocument2, context));
var load2 = getLoad(parse8, (dom, options3) => options3.xmlMode || options3._useHtmlParser2 ? esm_default(dom, options3) : renderWithParse5(dom));
var esm_default2 = load2([]);

// src/fetcher.ts
var fetcher = async (url, config) => {
  try {
...
// src/utils.ts
var getTickerData = async (ticker) => {
...
var getValuesFromSummaryTable = (html10) => {
  if (!html10) {
    throw new Error("No html provided");
  }
  if (!html10.includes("quote-summary")) {
    throw new Error("No summary found");
  }
  const $2 = load2(html10);
  const summary = $2("div#quote-summary");
  if (!summary) {
    throw new Error("No summary found");
  }
  ...
  const obj = merged?.reduce((acc, cur) => {
...
var statisticsSelector = (html10) => {
  const $2 = load2(html10);
  const statistics = $2('section[data-test="qsp-statistics"]');
  return statistics;
};
var getValuesFromStatisticsTable = (html10) => {
  const statistics = statisticsSelector(html10);
  if (!statistics) {
...
var getValuationMeasures = (statistics) => {
  const html10 = statistics.html();
  if (!html10)
    return;
  const $2 = load2(html10);
  const valuationMeasuresH2 = statistics.find('h2:contains("Valuation Measures")');
  ...
  const dates = headers.get().map((header) => $2(header).text()).filter((text3) => text3.includes("Current") || /\d{1,2}\/\d{1,2}\/\d{4}/.test(text3)).map((text3) => {
    if (text3.includes("Current")) {
      return "Current";
    }
    return text3;
  });
  let measuresByDate = dates.map((date) => {
    return {
      quarter: date
    };
  });
  ...
  for (const row of rows) {
    const label = $2(row).find("td:nth-child(1)")?.text();
    if (!label) {
      throw new Error("No label found");
    }
    const values = Array.from($2(row).children("td")).slice(1);
    for (const [index2, value] of values.entries()) {
      measuresByDate[index2][label] = $2(value).text();
    }
  }
  return measuresByDate;
};
var getFinancialAndTradingInfo = (statistics) => {
  const html10 = statistics.html();
  if (!html10)
    return;
  const $2 = load2(html10);
  const sections = statistics.find('div[class=""]:has(> h3, > table)').get().map((el, i) => {
    const div = $2(el);
    const h3 = div.find("h3").text().trim();
    const table = div.find("table");
    const rows = table?.find("tbody>tr").get();
    const values = rows?.map((row) => {
      const label = $2(row).find("td:nth-child(1)")?.text().trim();
      const value = $2(row).find("td:nth-child(2)")?.text().trim();
      return { label, value };
    });
    return {
      h3,
    ...
    section.values.forEach((value) => {
      acc[value.label] = isNaN(Number(value.value)) ? value.value : Number(value.value);
    });
    return acc;
  }, {});
  return sections;
...
if (import.meta.main) {
  ...
  program2.command("yapi <ticker>").option("-s, --saveTo <path>", "Specify the saveTo path").option("--verbose", "Print debug").description('Start the "yapi" CLI tool with the given ticker and optional saveTo path.').action(async (ticker, options3) => {
    const { saveTo, verbose } = options3;
    if (!ticker) {
      console.error("Please provide a ticker");
      throw new Error("Please provide a ticker");
    }
    const data2 = await getTickerData(ticker);
    const pkg = {
      ticker,
      data: data2
    };
